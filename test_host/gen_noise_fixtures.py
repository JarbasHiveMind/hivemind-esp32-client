#!/usr/bin/env python3
"""Generate Noise interop fixtures for the C protocol-v3 implementation.
...
``noiseprotocol`` library) using **fixed static and ephemeral keys**, and dumps
every wire byte as a C header. The C Noise initiator must reproduce
messages 1 and 3 byte-identically, decrypt message 2 and the responder
transport frames, and produce byte-identical initiator transport frames.

Usage:
    ~/.venvs/hivemind-v3/bin/python test_host/gen_noise_fixtures.py \
        > test/noise_fixtures.h
"""
import json
import os
import sys
...
def run_handshake(pattern: str, protocol_name: str):
    ...
    return {
        ...
        "handshake_hash": init.handshake_hash,
        "ct_c2s": ct_c2s, "ct_c2s_2": ct_c2s_2,
        "ct_s2c": ct_s2c, "ct_s2c_bin": ct_s2c_bin,
    }


def emit_bytes(name: str, data: bytes):
    if not data:  # KKpsk0 has no message 3
        print(f"static const uint8_t {name}[1] = {{0}};")
        print(f"#define {name.upper()}_LEN 0")
        print()
        return
    ...
    for i in range(0, len(data), 12):
        ...
        print(f"    {row},")
    print("};")
    print(f"#define {name.upper()}_LEN {len(data)}")
    print()


def emit_suite(tag: str, vectors: dict):
    for key, data in vectors.items():
        emit_bytes(f"fx_{tag}_{key}", data)
...
def main():
    ...
    print("/**")
    print(" * @file noise_fixtures.h")
    print(" * @brief Interop fixtures for the protocol-v3 Noise handshake,")
    print(" *        generated from the Python reference implementation")
    print(" *        (poorman_handshake.noise / noiseprotocol) with fixed keys.")
    print(" *")
    print(" * Regenerate with: python3 test_host/gen_noise_fixtures.py > test/noise_fixtures.h")
    print(" */")
    print("#ifndef NOISE_FIXTURES_H")
    print("#define NOISE_FIXTURES_H")
    print()
    print("#include <stdint.h>")
    print()
    emit_bytes("fx_psk", PSK)
    emit_bytes("fx_s_initiator_priv", S_INITIATOR)
    emit_bytes("fx_s_responder_pub", pub(S_RESPONDER))
    emit_bytes("fx_e_initiator_priv", E_INITIATOR)
    emit_bytes("fx_msg1_payload", MSG1_PAYLOAD)
    emit_bytes("fx_msg2_payload", MSG2_PAYLOAD)
    emit_bytes("fx_transport_c2s", TRANSPORT_C2S)
    emit_bytes("fx_transport_c2s_2", TRANSPORT_C2S_2)
    emit_bytes("fx_transport_s2c", TRANSPORT_S2C)
    emit_bytes("fx_transport_s2c_bin", TRANSPORT_S2C_BIN)
    emit_bytes("fx_canonical_hello", canonical_json(HELLO_PAYLOAD))
    emit_bytes("fx_canonical_handshake", canonical_json(HANDSHAKE_PAYLOAD))
    emit_suite("xx", xx)
    emit_suite("kk", kk)
    print("#endif /* NOISE_FIXTURES_H */")
